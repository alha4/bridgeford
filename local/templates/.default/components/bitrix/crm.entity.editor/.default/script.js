...
if(typeof BX.Crm.EntityEditor === "undefined")
{
	...
	BX.Crm.EntityEditor.prototype =
	{
       ...
       return this._entityTypeId;
		...
		initializeMapEvent : function() {

			const geoMap    = document.querySelector('#section_karta');
			
			this.bindEvent(geoMap, 'click', this.onMapClick);
		...
		initializeBuildingEvent : function() {
		  ...
		  this.bindEvent(this._buildingSelect, 'change', this.onBuildingChange);
			...
			const homeDelivered = this.nodeRadioChecked('UF_CRM_1540371563') ? this.nodeRadioChecked('UF_CRM_1540371563').value : this.getTextValue(this.node('UF_CRM_1540371563'));
		...
		onBuildingChange : function() {
				...
				const buildingValue =  this._buildingSelect ?
				                       this.nodeSelectValue(this._buildingSelect) : this.getTextValue(this.node('UF_CRM_1540371261836'));
    ...
    duplication2View : function(duplication) {
      ...
      if(paidExplotationTextValue) {
	    ...
	    if(BX('section_strategii_avtomaticheskogo_tsenoobrazovaniya') &&
            ...
            for(var item of competitors_list) {
							 ...
							 html+= `<span class="fields enumeration enumeration-checkbox field-item"><label for="item_${item['PRICE']}"><input id="item_${item['PRICE']}" ${mainAnchor.value == item['ID'] ? 'checked' : ''} name="comp_item" value="${item['ID']}" type="radio">`;
      ...
      if(e.target.nodeName == 'INPUT') {
		...
		initializeObjStatusEvent : function() {
			 ...
			 this._objStatus = this.node('UF_CRM_1545199624');
